...
**Регрессия перед коммитами:** для `movie-discovery` — `npm run build`, `npm run lint`, `npm run test:ci`; для `server` — `npm run build`, `npm run lint`, `npm test` (зелёный прогон зафиксирован при обновлении этого раздела).
