...
**Status:** `ROADMAP.md` in the repo is fully checked off; deferred work (ML v2, SMTP digest/outbox for alert rules, full alert matcher + digest email, etc.) is explicitly labeled there. Push: registration + outbound sends (dev self-test, release-reminder cron, dev `alerts/run` when a rule requests web push, all with VAPID). Email: outbound SMTP (`SMTP_*`) + dev `POST /api/email/dev/send-test` (JWT, `DEV_EMAIL_SEND_ENABLED`); release-reminder cron sends email when `channels.email` is set.
