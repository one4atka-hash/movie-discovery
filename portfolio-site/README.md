...
**Regression:** from repo root — Windows: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/verify-all.ps1`; Unix/macOS: `sh scripts/verify-all.sh` — runs build + lint + unit tests for `movie-discovery` and `server` (`server` uses `lint:ci`, same as GitHub Actions; see `ROADMAP.md` summary).
...
**CI:** `.github/workflows/ci.yml` runs gitleaks; full Angular lint/test/build; Nest `lint:ci` (ESLint without `--fix`) + build + unit tests; `docker build` for the API — no browser or API e2e jobs.
