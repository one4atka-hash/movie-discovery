...
**Status:** `ROADMAP.md` in the repo is fully checked off; deferred work (ML v2, SMTP, full alert matcher + digest email, etc.) is explicitly labeled there. Push: registration + outbound sends (dev self-test, release-reminder cron, dev `alerts/run` when a rule requests web push, all with VAPID).
