...
- [ ] **В каждой вкладке** добавить цель + “что нажать дальше” вверху страницы.
- [ ] **Discover**: purpose/instruction для “Сейчас в кино / Скоро выйдет / Рекомендации / Случайная подборка / Избранное”.
- [ ] **Today**: честные подсказки по constraints (убрать обещания того, чего нет).
- [ ] **Diary**: унифицировать RU/EN строки через `i18n.t(...)`.
- [ ] **Inbox**: объяснить 2 режима (local/server) без слова JWT в основном UI (JWT только в Advanced).
- [ ] **Account**: перестроить на блоки Profile/Connections/Preferences/Data/Advanced (без изменения функционала).
...
- [ ] **Connect to server**: логин/регистрация на server через `/api/auth/*`, токен хранить скрыто.
- [ ] **Connections UI**: Web Push connect/disconnect; Email setup (user‑grade, не dev endpoint).
...
- [ ] **Looks (themes)**: экран “Оформление” в аккаунте: список луков, превью, сделать активным.
- [ ] **Look editor**: layout (арт‑панель слева/справа/сверху/снизу или фон/градиент), шрифты (3 размера), акцентный цвет.
- [ ] **Доступ**:
  - [ ] Оформление публичной страницы — бесплатно.
  - [ ] Создание своего лука — символический донат (в MVP: ручной unlock/код; позже — платёжная интеграция).
